...
import {processHealthStatusRequest } from './healthStatusToolLogic.js';
 ...
 */
export const registerHealthStatusTool = async (server: McpServer): Promise<void> => {
  const toolName = "get_broker_health_status"; // The unique identifier for the tool
  const toolDescription =
    "Retrieve the status of the Broker and its components.";

  // Create registration context using the service
  const registrationContext = requestContextService.createRequestContext({
    operation: 'Get Broker health status',
    toolName: toolName,
    module: 'HealthStatusRegistration'
  });
      ...
      server.tool(
        ...
        async () => {
          ...
          const handlerContext = requestContextService.createRequestContext({
            ...
            operation: 'HealthStatusRequest',
            toolName: toolName
          });
          logger.debug("Handling health status tool request", handlerContext);

          // Wrap the handler logic in tryCatch for robust error handling
          return await ErrorHandler.tryCatch(
            async () => {
              // Delegate the core processing logic, passing the context
              const response = await processHealthStatusRequest(handlerContext);
              logger.debug("Health status request processed successfully", handlerContext);

              // Return the response in the standard MCP tool result format
              return {
                content: [{
                  type: "text", // Content type is text
                  // The actual content is a JSON string representing the HealthStatusResponse
            ...
            },
            {
              ...
              operation: 'processing health status message handler',
              ...
              errorMapper: (error) => new McpError(
                ...
                `Error processing health status message tool: ${error instanceof Error ? error.message : 'Unknown error'}`,
