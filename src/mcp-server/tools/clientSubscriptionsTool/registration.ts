...
import { ClientSubscriptionsInputSchema, ClientSubscriptionsInput, ClientSubscriptionsResponse } from './clientSubscriptionsToolLogic.js';
...
import { processClientSubscriptionsMessage } from './clientSubscriptionsToolLogic.js';
 ...
 */
export const registerClientSubscriptionsTool = async (server: McpServer): Promise<void> => {
  const toolName = "client_subscriptions"; // The unique identifier for the tool
  const toolDescription =
    "Retrieves the subscriptions of a specific client, using its client ID";

  // Create registration context using the service
  const registrationContext = requestContextService.createRequestContext({
    operation: 'Request client subscriptions',
    toolName: toolName,
    module: 'ClientSubscriptionsRegistration'
  });
  ...
  await ErrorHandler.tryCatch(
    async () => {
      ...
      server.tool(
        ...
        ClientSubscriptionsInputSchema.shape,
        // --- Tool Handler ---
        ...
        async (params: ClientSubscriptionsInput) => {
          // Create handler context using the service
          const handlerContext = requestContextService.createRequestContext({
            parentContext: registrationContext, // Link to registration context
            operation: 'ClientSubscriptionsRequest',
            toolName: toolName,
            params: params // Include relevant request details
          });
          logger.debug("Handling client subscriptions tool request", handlerContext);

          // Wrap the handler logic in tryCatch for robust error handling
          return await ErrorHandler.tryCatch(
            async () => {
              // Delegate the core processing logic, passing the context
              const response = await processClientSubscriptionsMessage(params, handlerContext);
              logger.debug("Client subscriptions request processed successfully", handlerContext);

              // Return the response in the standard MCP tool result format
              return {
                content: [{
                  type: "text", // Content type is text
                  // The actual content is a JSON string representing the ClientSubscriptionsResponse
